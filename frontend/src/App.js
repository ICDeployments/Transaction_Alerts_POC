...
import { Inbox, Search, FolderOpen, Users, ChevronDown, ExternalLink, Sparkles, CheckCircle, AlertOctagon, XCircle, X, TrendingUp, Database } from 'lucide-react';
...
function App() {
    ...
    const [allAlerts] = useState(mockAlerts);
    ...
    const [newlyArrivedAlerts, setNewlyArrivedAlerts] = useState(new Set());
    ...
    const handleAlertStatusChange = (alertId, newStatus, action, comments) => {
        const alert = alerts.find(a => a.alert_id === alertId);
        if (!alert)
            return;
        // Update alerts array
        setAlerts(prevAlerts => prevAlerts.map(a => a.alert_id === alertId
            ? { ...a, status: newStatus }
            : a));
    ...
    const handleTabChange = (alertId, tabName) => {
    ...
    const saveToAdvisoryDB = (alert, newStatus, action, comments) => {
    ...
    const fetchAIAnalysis = async (alertId) => {
        const alert = alerts.find(a => a.alert_id === alertId);
        if (!alert)
            return;
    ...
    const getRiskScoreColor = (score) => {
        if (score >= 75)
            return 'bg-gradient-to-br from-red-500 to-red-600 text-white shadow-glow-red';
        if (score >= 50)
            return 'bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-glow-blue';
    ...
    return (_jsxs("div", { className: "h-[calc(100vh-5rem)] flex bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50", children: [_jsxs("div", { className: "w-16 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 flex flex-col items-center py-5 space-y-6 shadow-strong border-r border-slate-700", children: [_jsx("div", { className: "w-full h-px bg-gradient-to-r from-transparent via-slate-600 to-transparent" }), _jsxs("button", { className: "text-slate-400 hover:text-white p-2.5 rounded-lg hover:bg-slate-700 transition-all hover:scale-110 active:scale-95 group relative", title: "Inbox", children: [_jsx(Inbox, { size: 22 }), _jsx("span", { className: "absolute left-full ml-3 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap", children: "Inbox" })] }), _jsxs("button", { className: "text-slate-400 hover:text-white p-2.5 rounded-lg hover:bg-slate-700 transition-all hover:scale-110 active:scale-95 group relative", title: "Search", children: [_jsx(Search, { size: 22 }), _jsx("span", { className: "absolute left-full ml-3 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap", children: "Search" })] }), _jsxs("button", { className: "text-slate-400 hover:text-white p-2.5 rounded-lg hover:bg-slate-700 transition-all hover:scale-110 active:scale-95 group relative", title: "Case Manager", children: [_jsx(FolderOpen, { size: 22 }), _jsx("span", { className: "absolute left-full ml-3 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap", children: "Cases" })] }), _jsxs("button", { className: "text-slate-400 hover:text-white p-2.5 rounded-lg hover:bg-slate-700 transition-all hover:scale-110 active:scale-95 group relative", title: "Entity Insights", children: [_jsx(Users, { size: 22 }), _jsx("span", { className: "absolute left-full ml-3 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap", children: "Entities" })] }), _jsx("div", { className: "w-full h-px bg-gradient-to-r from-transparent via-slate-600 to-transparent" }), _jsxs("button", { onClick: () => setShowAdvisoryDB(!showAdvisoryDB), className: `p-2.5 rounded-lg transition-all hover:scale-110 active:scale-95 group relative ${showAdvisoryDB
                            ? 'text-white bg-blue-600'
                            : 'text-slate-400 hover:text-white hover:bg-slate-700'}`, title: "Advisory Database", children: [_jsx(Database, { size: 22 }), _jsx("span", { className: "absolute left-full ml-3 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap", children: "Advisory DB" }), advisoryRecords.length > 0 && (_jsx("span", { className: "absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center", children: advisoryRecords.length }))] })] }), _jsxs("div", { className: "flex-1 flex flex-col", children: [showAdvisoryDB ? (_jsx(AdvisoryDatabase, { records: advisoryRecords, onBack: () => setShowAdvisoryDB(false) })) : (_jsxs(_Fragment, { children: [_jsx("div", { className: "flex-1 flex overflow-hidden", children: _jsxs("div", { className: "transition-all duration-300 overflow-y-auto w-full", children: [_jsx("div", { className: "bg-white border-b-2 border-slate-200 shadow-md", children: _jsxs("div", { className: "px-6 py-4", children: [_jsx("div", { className: "flex items-center justify-between mb-3", children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("h2", { className: "text-16px font-bold text-slate-900", children: "Key Metrics" }), _jsxs("button", { onClick: () => setIsDashboardExpanded(true), className: "px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-11px font-bold rounded-lg transition-all hover:scale-105 active:scale-95 flex items-center gap-2 shadow-md", children: [_jsx(Sparkles, { size: 14 }), "View Full Analytics"] })] }) }), _jsxs("div", { className: "grid grid-cols-6 gap-3 mb-4", children: [_jsxs("div", { className: "bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-3 border border-blue-200", children: [_jsx("div", { className: "text-10px font-bold text-blue-600 uppercase tracking-wide mb-1", children: "Total Alerts" }), _jsx("div", { className: "text-24px font-bold text-blue-900", children: alerts.length })] }), _jsxs("div", { className: "bg-gradient-to-br from-red-50 to-red-100 rounded-lg p-3 border border-red-200", children: [_jsx("div", { className: "text-10px font-bold text-red-600 uppercase tracking-wide mb-1", children: "High Risk" }), _jsx("div", { className: "text-24px font-bold text-red-900", children: alerts.filter(a => (a.risk_score || 0) >= 75).length })] }), _jsxs("div", { className: "bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg p-3 border border-amber-200", children: [_jsx("div", { className: "text-10px font-bold text-amber-600 uppercase tracking-wide mb-1", children: "Overdue" }), _jsx("div", { className: "text-24px font-bold text-amber-900", children: alerts.filter(a => a.sla_status === 'Overdue').length })] }), _jsxs("div", { className: "bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-lg p-3 border border-emerald-200", children: [_jsx("div", { className: "text-10px font-bold text-emerald-600 uppercase tracking-wide mb-1", children: "Closed" }), _jsx("div", { className: "text-24px font-bold text-emerald-900", children: alerts.filter(a => a.status === 'Closed').length })] }), _jsxs("div", { className: "bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-3 border border-purple-200", children: [_jsx("div", { className: "text-10px font-bold text-purple-600 uppercase tracking-wide mb-1", children: "In Review" }), _jsx("div", { className: "text-24px font-bold text-purple-900", children: alerts.filter(a => a.status === 'In Review').length })] }), _jsxs("div", { className: "bg-gradient-to-br from-slate-50 to-slate-100 rounded-lg p-3 border border-slate-200", children: [_jsx("div", { className: "text-10px font-bold text-slate-600 uppercase tracking-wide mb-1", children: "Open" }), _jsx("div", { className: "text-24px font-bold text-slate-900", children: alerts.filter(a => a.status === 'Open').length })] })] }), _jsxs("div", { className: "grid grid-cols-2 gap-4", children: [_jsxs("div", { className: "bg-gradient-to-br from-slate-50 to-blue-50 rounded-lg p-4 border border-slate-200", children: [_jsx("h3", { className: "text-13px font-bold text-slate-900 mb-2", children: "Status Distribution" }), _jsx(ResponsiveContainer, { width: "100%", height: 180, children: _jsxs(PieChart, { children: [_jsx(Pie, { data: [
                                                                                    ...
                                                                                    ].filter(item => item.value > 0), cx: "50%", cy: "50%", labelLine: false, label: ({ name, percent }) => `${name} ${((percent || 0) * 100).toFixed(0)}%`, outerRadius: 60, dataKey: "value", children: [
                                                                                        { name: 'Open', value: alerts.filter(a => a.status === 'Open').length, color: '#3b82f6' },
                                                                                        { name: 'In Review', value: alerts.filter(a => a.status === 'In Review').length, color: '#f59e0b' },
                                                                                        { name: 'Escalated', value: alerts.filter(a => a.status === 'Escalated').length, color: '#ef4444' },
                                                                                        { name: 'Closed', value: alerts.filter(a => a.status === 'Closed').length, color: '#10b981' },
                                                                                    ].filter(item => item.value > 0).map((entry, index) => (_jsx(Cell, { fill: entry.color }, `cell-${index}`))) }), _jsx(Tooltip, {})] }) })] }), _jsxs("div", { className: "bg-gradient-to-br from-slate-50 to-blue-50 rounded-lg p-4 border border-slate-200", children: [_jsx("h3", { className: "text-13px font-bold text-slate-900 mb-2", children: "Severity Distribution" }), _jsx(ResponsiveContainer, { width: "100%", height: 180, children: _jsxs(PieChart, { children: [_jsx(Pie, { data: [
                                                                                        ...
                                                                                        { name: 'Medium', value: alerts.filter(a => a.severity === 'Medium').length, color: '#f59e0b' },
                                                                                        { name: 'Low', value: alerts.filter(a => a.severity === 'Low').length, color: '#10b981' },
                                                                                    ].filter(item => item.value > 0).map((entry, index) => (_jsx(Cell, { fill: entry.color }, `cell-${index}`))) }), _jsx(Tooltip, {})] }) })] })] })] }) }), _jsx("div", { className: "bg-white", children: _jsxs("table", { className: "w-full text-12px", children: [_jsx("thead", { className: "bg-gradient-to-r from-slate-50 to-slate-100 sticky top-0 z-10 shadow-sm", children: _jsxs("tr", { className: "text-11px text-slate-700 border-b-2 border-slate-200", children: [_jsx("th", { className: "px-5 py-3.5 text-left font-bold sticky left-0 bg-gradient-to-r from-slate-50 to-slate-100 uppercase tracking-wide", children: "Alert ID" }), _jsx("th", { className: "px-5 py-3.5 text-left font-bold uppercase tracking-wide", children: "Entity Name" }), _jsx("th", { className: "px-5 py-3.5 text-center font-bold uppercase tracking-wide", children: "Risk Score" }), _jsx("th", { className: "px-5 py-3.5 text-left font-bold uppercase tracking-wide", children: "Alert Type" }), _jsx("th", { className: "px-5 py-3.5 text-center font-bold uppercase tracking-wide", children: "Age (Days)" }), _jsx("th", { className: "px-5 py-3.5 text-left font-bold uppercase tracking-wide", children: "Status" }), _jsx("th", { className: "px-5 py-3.5 text-left font-bold uppercase tracking-wide", children: "Assigned To" }), _jsx("th", { className: "px-5 py-3.5 text-left font-bold uppercase tracking-wide", children: "SLA Status" })] }) }), _jsx("tbody", { children: alerts.map((alert) => (_jsxs("tr", { onClick: () => setSelectedAlert(alert), className: `border-b border-slate-100 cursor-pointer transition-all duration-200 ${selectedAlert?.alert_id === alert.alert_id
                                                                ? 'bg-gradient-to-r from-blue-50 to-blue-100 border-l-4 border-l-blue-600 shadow-soft'
                                                                : 'bg-white hover:bg-slate-50 hover:shadow-soft'}`, children: [_jsx("td", { className: "px-5 py-3.5 sticky left-0 bg-inherit", children: _jsxs("span", { className: "text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1.5 font-bold transition-colors group", children: [newlyArrivedAlerts.has(alert.alert_id) && (_jsxs("span", { className: "relative flex h-3 w-3", children: [_jsx("span", { className: "animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75" }), _jsx("span", { className: "relative inline-flex rounded-full h-3 w-3 bg-red-500" })] })), alert.alert_id, _jsx(ExternalLink, { size: 12, className: "opacity-0 group-hover:opacity-100 transition-opacity" })] }) }), _jsx("td", { className: "px-5 py-3.5 font-semibold text-slate-900", children: alert.entity_name || alert.transaction.customer_name }), _jsx("td", { className: "px-5 py-3.5 text-center", children: _jsx("span", { className: `inline-flex items-center justify-center px-3 py-1.5 rounded-lg font-bold text-12px min-w-[50px] shadow-sm ${getRiskScoreColor(alert.risk_score || 0)}`, children: alert.risk_score || 0 }) }), _jsx("td", { className: "px-5 py-3.5 text-slate-700 font-medium", children: alert.alert_type || 'Unknown' }), _jsx("td", { className: "px-5 py-3.5 text-center font-bold text-slate-900", children: _jsx("span", { className: "inline-flex items-center justify-center w-10 h-10 rounded-full bg-slate-100 text-slate-700", children: alert.age_days || 0 }) }), _jsx("td", { className: "px-5 py-3.5", children: _jsx("span", { className: `inline-flex items-center px-3 py-1 rounded-full text-11px font-bold uppercase tracking-wide ${getStatusColor(alert.status)}`, children: alert.status }) }), _jsx("td", { className: "px-5 py-3.5 text-slate-700 font-medium", children: alert.assigned_to || 'Unassigned' }), _jsx("td", { className: "px-5 py-3.5", children: _jsx("span", { className: `inline-flex items-center px-3 py-1 rounded-full text-11px font-bold uppercase tracking-wide ${getSLAColor(alert.sla_status)}`, children: alert.sla_status || 'On Time' }) })] }, alert.alert_id))) })] }) })] }) }), selectedAlert && (_jsx("div", { className: "fixed inset-0 z-50 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-8 animate-fade-in", children: _jsxs("div", { className: "w-full max-w-6xl h-[95vh] bg-white rounded-2xl shadow-strong overflow-hidden animate-slide-up flex flex-col", children: [_jsxs("div", { className: "bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 text-white px-6 py-5 flex items-center justify-between shadow-strong", children: [_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: `w-16 h-16 rounded-xl flex items-center justify-center font-bold text-20px shadow-lg ${(selectedAlert.risk_score || 0) >= 75
                                                                ? 'bg-gradient-to-br from-red-500 to-red-600'
                                                                : (selectedAlert.risk_score || 0) >= 50
                                                                    ? 'bg-gradient-to-br from-amber-500 to-orange-600'
                                                                    : 'bg-gradient-to-br from-emerald-500 to-emerald-600'}`, children: selectedAlert.risk_score || 0 }), _jsxs("div", { children: [_jsx("h2", { className: "text-20px font-bold", children: selectedAlert.alert_id }), _jsxs("p", { className: "text-13px text-slate-300 mt-1", children: [selectedAlert.entity_name || selectedAlert.transaction.customer_name, " \u2022 ", selectedAlert.alert_type] })] })] }), _jsx("button", { onClick: () => setSelectedAlert(null), className: "w-10 h-10 rounded-full bg-white bg-opacity-20 hover:bg-opacity-30 flex items-center justify-center transition-all hover:scale-110 active:scale-95", title: "Close", children: _jsx(X, { size: 24, className: "text-white" }) })] }), _jsx("div", { className: "flex-1 overflow-y-auto bg-gradient-to-br from-slate-50 to-blue-50", children: _jsx(AlertDetail, { alert: selectedAlert, onStatusChange: handleAlertStatusChange, onTabChange: handleTabChange }) })] }) })), selectedAlert && !isAIExpanded && (_jsxs("div", { className: "fixed bottom-8 right-8 z-50", children: [loadingAlerts[selectedAlert.alert_id] && (_jsx("div", { className: "animate-slide-up", children: _jsxs("div", { className: "relative", children: [_jsx("div", { className: "absolute inset-0 bg-gradient-to-r from-indigo-400 to-purple-400 rounded-full blur-2xl opacity-40 animate-pulse-soft" }), _jsx("div", { className: "relative w-20 h-20 bg-gradient-to-br from-slate-700 via-slate-800 to-slate-900 rounded-full flex items-center justify-center shadow-strong", children: _jsxs("div", { className: "flex items-center gap-1.5", children: [_jsx("div", { className: "w-2 h-2 bg-white rounded-full animate-thinking-dot-1" }), _jsx("div", { className: "w-2 h-2 bg-white rounded-full animate-thinking-dot-2" }), _jsx("div", { className: "w-2 h-2 bg-white rounded-full animate-thinking-dot-3" })] }) }), _jsxs("div", { className: "absolute bottom-full mb-4 right-0 bg-slate-800 text-white px-4 py-2.5 rounded-lg text-13px font-medium whitespace-nowrap shadow-strong", children: [_jsx("span", { children: "Analyzing transaction..." }), _jsx("div", { className: "absolute top-full right-6 w-0 h-0 border-l-8 border-r-8 border-t-8 border-transparent border-t-slate-800" })] })] }) })), !loadingAlerts[selectedAlert.alert_id] && showAIBubble && (_jsx("div", { className: "animate-bounce-in cursor-pointer group", onClick: () => setIsAIExpanded(true), children: _jsxs("div", { className: "relative", children: [_jsx("div", { className: "absolute inset-0 bg-gradient-to-r from-emerald-500 to-blue-600 rounded-full blur-xl opacity-60 group-hover:opacity-80 transition-opacity animate-pulse-soft" }), _jsx("div", { className: "relative w-20 h-20 bg-gradient-to-br from-emerald-500 via-blue-600 to-purple-700 rounded-full flex items-center justify-center shadow-strong group-hover:scale-110 transition-all", children: _jsx(Sparkles, { size: 36, className: "text-white animate-pulse-soft" }) }), _jsx("div", { className: "absolute -top-2 -right-2 w-8 h-8 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-full flex items-center justify-center text-white font-bold text-11px shadow-glow-green border-2 border-white animate-ping-once", children: "\u2713" }), _jsxs("div", { className: "absolute bottom-full mb-4 right-0 bg-gradient-to-r from-emerald-600 to-blue-700 text-white px-4 py-3 rounded-lg text-13px font-semibold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity shadow-strong max-w-xs", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Sparkles, { size: 16 }), _jsx("span", { children: "Analysis complete \u2014 Click to view insights" })] }), _jsx("div", { className: "absolute top-full right-6 w-0 h-0 border-l-8 border-r-8 border-t-8 border-transparent border-t-blue-700" })] })] }) }))] })), selectedAlert && isAIExpanded && (_jsx("div", { className: "fixed inset-0 z-50 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-8 animate-fade-in", children: _jsxs("div", { className: "w-full max-w-4xl h-[90vh] bg-white rounded-2xl shadow-strong overflow-hidden animate-slide-up flex flex-col", children: [_jsxs("div", { className: "bg-gradient-to-r from-blue-600 via-blue-700 to-purple-700 text-white px-6 py-5 flex items-center justify-between shadow-strong", children: [_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: "w-12 h-12 bg-white bg-opacity-20 rounded-xl flex items-center justify-center backdrop-blur-sm", children: _jsx(Sparkles, { size: 24, className: "text-white" }) }), _jsxs("div", { children: [_jsx("h2", { className: "text-20px font-bold", children: "Intelligent Fraud Analysis" }), _jsxs("p", { className: "text-13px text-blue-100", children: ["Alert: ", selectedAlert.alert_id] })] })] }), _jsx("button", { onClick: () => setIsAIExpanded(false), className: "w-10 h-10 rounded-full bg-white bg-opacity-20 hover:bg-opacity-30 flex items-center justify-center transition-all hover:scale-110 active:scale-95", title: "Minimize", children: _jsx(ChevronDown, { size: 24, className: "text-white" }) })] }), _jsx("div", { className: "flex-1 overflow-y-auto", children: _jsx(RecommendationPanel, { alert: selectedAlert, onDecision: handleDecision, aiAnalysisCache: aiAnalysisCache, setAiAnalysisCache: setAiAnalysisCache }) })] }) })), isDashboardExpanded && (_jsx("div", { className: "fixed inset-0 z-50 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-8 animate-fade-in", children: _jsxs("div", { className: "w-full max-w-7xl h-[95vh] bg-white rounded-2xl shadow-strong overflow-hidden animate-slide-up flex flex-col", children: [_jsxs("div", { className: "bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700 text-white px-6 py-5 flex items-center justify-between shadow-strong", children: [_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: "w-12 h-12 bg-white bg-opacity-20 rounded-xl flex items-center justify-center backdrop-blur-sm", children: _jsx(TrendingUp, { size: 24, className: "text-white" }) }), _jsxs("div", { children: [_jsx("h2", { className: "text-20px font-bold", children: "Full Analytics Dashboard" }), _jsx("p", { className: "text-13px text-blue-100", children: "Complete metrics and visualizations" })] })] }), _jsx("button", { onClick: () => setIsDashboardExpanded(false), className: "w-10 h-10 rounded-full bg-white bg-opacity-20 hover:bg-opacity-30 flex items-center justify-center transition-all hover:scale-110 active:scale-95", title: "Close", children: _jsx(X, { size: 24, className: "text-white" }) })] }), _jsx("div", { className: "flex-1 overflow-y-auto bg-gradient-to-br from-slate-50 to-blue-50", children: _jsx(Dashboard, { alerts: alerts }) })] }) }))] })), actionNotification?.show && (_jsx("div", { className: "fixed top-24 right-8 z-[60] animate-slide-down", children: _jsxs("div", { className: "bg-white rounded-xl shadow-strong border-2 border-emerald-200 p-5 min-w-[350px] animate-bounce-in", children: [_jsxs("div", { className: "flex items-start gap-4", children: [_jsxs("div", { className: `w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${actionNotification.status === 'Closed'
                                                ? 'bg-gradient-to-br from-emerald-500 to-emerald-600'
                                                : actionNotification.status === 'Escalated'
                                                    ? 'bg-gradient-to-br from-red-500 to-red-600'
                                                    : 'bg-gradient-to-br from-amber-500 to-amber-600'}`, children: [actionNotification.status === 'Closed' && _jsx(CheckCircle, { size: 24, className: "text-white" }), actionNotification.status === 'Escalated' && _jsx(AlertOctagon, { size: 24, className: "text-white" }), actionNotification.status === 'In Review' && _jsx(Search, { size: 24, className: "text-white" })] }), _jsxs("div", { className: "flex-1", children: [_jsx("div", { className: "font-bold text-slate-900 text-15px mb-1", children: "Action Completed" }), _jsx("div", { className: "text-13px text-slate-600 mb-2", children: actionNotification.action }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-11px font-bold text-slate-500 uppercase tracking-wide", children: "Alert:" }), _jsx("span", { className: "text-12px font-mono font-bold text-blue-600", children: actionNotification.alertId })] }), _jsxs("div", { className: "flex items-center gap-2 mt-1", children: [_jsx("span", { className: "text-11px font-bold text-slate-500 uppercase tracking-wide", children: "New Status:" }), _jsx("span", { className: `inline-flex px-2 py-0.5 rounded-md text-11px font-bold ${actionNotification.status === 'Closed'
                                                                ? 'bg-emerald-100 text-emerald-700 border border-emerald-300'
                                                                : actionNotification.status === 'Escalated'
                                                                    ? 'bg-red-100 text-red-700 border border-red-300'
                                                                    : 'bg-amber-100 text-amber-700 border border-amber-300'}`, children: actionNotification.status })] })] }), _jsx("button", { onClick: () => setActionNotification(null), className: "w-6 h-6 rounded-full hover:bg-slate-100 flex items-center justify-center transition-colors flex-shrink-0", children: _jsx(XCircle, { size: 16, className: "text-slate-400" }) })] }), _jsx("div", { className: "mt-3 h-1 bg-slate-100 rounded-full overflow-hidden", children: _jsx("div", { className: "h-full bg-gradient-to-r from-emerald-500 to-blue-600 rounded-full animate-progress-bar" }) })] }) }))] })] }));
}
export default App;
